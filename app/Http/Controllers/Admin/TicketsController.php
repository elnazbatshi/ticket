...
class TicketsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Ticket::query()->with(['status', 'priority', 'category','author', 'assigned_to_user', 'comments'])
            ...
            $table->addColumn('view_link', function ($row) {
                return route('tickets.show', $row->id);
            });
       ...
       if ($userRoles && !in_array('Admin', $userRoles)) {

            if (in_array('Customer', $userRoles)) {
                $categories = Category::whereHas('customers', function ($q) {
                    $q->where('id', '=', auth()->user()->id);
                })->get();

            } elseif (in_array('Agent', $userRoles)) {
                $categories = Category::whereHas('agents', function ($q) {
                    $q->where('id', '=', auth()->user()->id);
                })->get();
            }
        } else {
    ...
    {
        abort_if(Gate::denies('admin_ticket_create'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        $author_name = User::all()->pluck('name', 'id')->prepend(trans('global.pleaseSelect'), '');
        ...
        return view('admin.tickets.create', compact('statuses', 'priorities', 'categories', 'assigned_to_users','author_name'));
    }
...
//            ->prepend(trans('global.pleaseSelect'), '');
        ...
        $assigned_to_users = Ticket::with('category')->findOrFail($ticket->id)->category->agents->pluck('name', 'id');
    ...
    public function storeComment(Request $request, Ticket $ticket)
    {
        ...
        $customerEmail = $ticket->AuthorEmail;
        $agentData = $user;
//        dd($ticket->title);
        Mail::send(new commentMail($agentData,$customerEmail, $ticket));
